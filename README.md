...
- This dataset includes air quality data that was collected every three days in Atlanta, GA throughout the year 2024, starting on 01/01/2024. The air pollution data was sourced from the EPA's Air Quality System. There are a total of 9 air quality monitoring sites in Atlanta which collect PM2.5 data. The some of the variables that comprise the dataset are the `Date`, the daily mean PM2.5 concentrations (ug/m3 LC) - `Daily.Mean.PM2.5.Concentrations`, the daily air quality index (AQI) value - `Daily.AQI.Value`, air monitor site ID - `Site.ID`, site name - `Local.Site.Name`, and `County`. 

## Outputs: 
- **Table:** `output/RDS/Table.rds`
   - Monthly Average PM2.5 (µg/m³) For Each Air Monitoring Site Location (2024). 
- **Boxplot:** `output/RDS/Boxplot.rds`
...
- `code/01_clean_data.R`
  ...
  - Saves data as `Average_PM2.5.rds` and `Fulton_data.rds` in /output folder
